...
try {
    ...
    # Lösche diese Datei selbst (wird von WMI aufgerufen) - ABER NUR WENN ERFOLGREICH
    ...
    # Lösche alle temporären Lunevo-Dateien (cleanup) - ABER NICHT DEN LOG
    # Get-ChildItem -Path "$env:TEMP\Lunevo" -ErrorAction SilentlyContinue | 
    #     Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) -and $_.Name -ne "startup_chain.log" } | 
